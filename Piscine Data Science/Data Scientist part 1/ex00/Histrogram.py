...
def draw_single(path):
    ...
    fig, axes = plt.subplots(nrows=nb_rows, ncols=min(nb_cols, 5), figsize=(15, 2.5 * nb_rows))
    axes = axes.flatten()
    
    for i, column in enumerate(df.columns):
        if i < len(axes):
            axes[i].hist(df[column], bins=40, color='darkseagreen', label='Knight')
            axes[i].set_title(column)
            axes[i].legend(loc='upper right')

    plt.tight_layout()
    plt.savefig(f"Histrogram_single.jpg")
...
def draw_double(path):
    ...
    for i, column in enumerate(df.columns):
        if i < len(axes):
            ...
            axes[i].hist(subset_jedi, bins=40, color='blue', alpha=0.5, label='Jedi')
            
            subset_sith = df[df['knight'] == 'Sith'][column]
            axes[i].hist(subset_sith, bins=40, color='tomato', alpha=0.5, label='Sith')
